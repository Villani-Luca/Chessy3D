...
import numpy as np
from chessy.chessboard_localization.localization import draw_chessboard_squares, find_best_chessboard_polygon, find_best_fitting_enclosing_square, find_chessboard_squares, find_best_squares_hough, process_contours, sort_quadrilateral_approx
...
def auto_chessboard_localization_alt(image, resized_image):
    ...
    for candidate_idx, corners in enumerate(corners_list):
        ...
        if corners is not None:
            ...
            final_candidates.append((corners_original, best_squares_number, hough_image, best_valid_square_image, mask_image, working_image))
    ...
    if len(final_candidates) > 0:
        ...
        _, _, hough_image, polygons_image, mask_image, working_image = final_candidates[i]

        return output_with_corners, corners_list, squares_data, resized_image, canny_image, hough_image, polygons_image, upsize_factor, mask_image, working_image
    else:
        return resized_image, [], [], resized_image, canny_image, resized_image, resized_image, upsize_factor, resized_image, resized_image