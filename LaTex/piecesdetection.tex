Our approach in the object detection has been a global one.
We chose to use a global approach in order to not segment every image in single pieces.
In this way we could avoid having the empty class and the duplication of object detected that could have been present thanks to the duplication of the piece due to overlaps created during the image extraction (cropping) process.
\newline
...
As we can see in the table below, the model achieved high precision and recall values across the three training runs,
...
\newline
After that we decided to find better datasets for training the model and started a training run from scratch.
...
We started the first training with only one dataset, Chess Piece Detection \cite{cpd:dataset:chesspiecedetection}, which contains 14808 images of chess pieces in various conditions.
After 600 epochs we achieve some good results, but not good enough for our task.
...
\newline
After 200 epochs we started to see some good results in the training process, with the loss function decreasing and the precision and recall values increasing even more than the previous runs.
\begin{figure}[ht]
\centering
...
\newline
At the end of our research we used ChessRed2k as validation dataset for all the trainings done.
We have saved up to 5 checkpoints (4 from our training and 1 found online) and estimated their capabilities.
For the one found online, we talked to the owner and asked if we could use it and also if he could give us the same dataset but we couldn't reach the same level of accuracy.
\begin{table}[ht]
\centering
\caption{Final validation loss values for all the trainings done.}
\label{tab:yolov8:performance:final}
\resizebox{\linewidth}{!}{%
\begin{tabular}{lcccc}
\textbf{Run} & \textbf{P} & \textbf{R} & \textbf{mAP50} & \textbf{mAP50-95} \\
First Run - Small Dataset & $\sim$0.372 & $\sim$0.218 & $\sim$0.202 & $\sim$0.115\\
Only Chess Piece Detection Dataset & $\sim$0.827 & $\sim$0.798 & $\sim$0.873 & $\sim$0.681\\
Three Merged Dataset - From Scratch & $\sim$0.874 & $\sim$0.855 & $\sim$0.913 & $\sim$0.700\\
Three Merged Dataset - From PreTrained& $\sim$0.833 & $\sim$0.847 & $\sim$0.911 & $\sim$0.735\\
CheckPoint Online & $\sim$0.921 & $\sim$0.907 & $\sim$0.964 & $\sim$0.718 
\end{tabular}
}
\end{table}
